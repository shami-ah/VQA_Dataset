...
def is_malay_text(text: str, min_malay_ratio: float = 0.7) -> bool:
    """
    Check if text is predominantly Malay (Latin script with Malay characteristics)
    
    Args:
        text: Text to analyze
        min_malay_ratio: Minimum ratio of Latin characters required
        
    Returns:
        True if text is predominantly Malay
    """
    ...
    # Count Latin alphabetic characters (basic check for Malay using Latin script)
    ...
    # Additional check for common Malay words or patterns
    malay_indicators = [
        'yang', 'dan', 'ini', 'itu', 'adalah', 'untuk', 'dengan', 'pada',
        'dari', 'akan', 'oleh', 'dalam', 'tidak', 'ada', 'juga', 'atau',
        'sebagai', 'dapat', 'sudah', 'masih', 'telah', 'bagi', 'satu',
        'dua', 'tiga', 'empat', 'lima', 'enam', 'tujuh', 'lapan', 'sembilan'
    ]
    
    text_lower = text.lower()
    has_malay_indicators = any(indicator in text_lower for indicator in malay_indicators)
    
    # Return true if both Latin script and Malay indicators are present
    return latin_ratio >= min_malay_ratio and (has_malay_indicators or latin_ratio >= 0.9)


def has_educational_keywords(text: str) -> bool:
    """
    Check if text contains Malay educational keywords that indicate learning content
    ...
    educational_keywords = [
        'belajar', 'pendidikan', 'sekolah', 'pelajaran', 'mengajar', 'pelajar',
        'matematik', 'sains', 'fizik', 'kimia', 'biologi', 'sejarah',
        'geografi', 'sastera', 'tatabahasa', 'kosa kata', 'persamaan',
        'formula', 'masalah', 'penyelesaian', 'jawapan', 'soalan', 'peperiksaan',
        'ujian', 'kuiz', 'kerja rumah', 'tugasan', 'bab', 'unit',
        'rajah', 'graf', 'carta', 'jadual', 'gambar', 'contoh',
        'ilmu', 'mata pelajaran', 'kursus', 'profesor', 'guru', 'pengajaran'
    ]
...
def detect_watermarks(image_path: str, ocr_text: str) -> Tuple[bool, str]:
    ...
    getty_indicators = [
        'getty', 'gettyimages', 'getty images', 'tanda air'
    ]
    ...
    # Malay watermark terms
    malay_watermarks = [
        'tanda air', 'hak cipta', 'sampel', 'pratonton',
        'contoh', 'demonstrasi', 'demo'
    ]
    ...
    # Malay watermark detection
    for watermark in malay_watermarks:
        if watermark in text_lower:
            return True, f"Malay Watermark ({watermark})"
...
def is_high_quality_educational_image(image_path: str, ocr_text: str) -> Dict:
    """
    Comprehensive quality check for Malay educational images
    ...
    # Check if predominantly Malay text
    assessment['malay_text'] = is_malay_text(ocr_text)
    if not assessment['malay_text']:
        assessment['is_quality'] = False
        assessment['reasons'].append("Not predominantly Malay text")
...
def process_images(source_dir, save_dir):
    """
    Enhanced OCR-filter for Malay educational images with watermark detection.
    Only high-quality images with Malay educational text and no watermarks are saved.
    """
    os.makedirs(save_dir, exist_ok=True)

    # Initialize Malay EasyOCR reader
    try:
        reader = easyocr.Reader(['ms'], gpu=False)  # Use CPU for better compatibility
    ...
    print("🔍 Starting enhanced Malay OCR processing...")
    print("✅ Filtering for: Malay text + Educational content + No watermarks")
    ...
    for root, _, files in os.walk(source_dir):
        for fname in files:
            ...
            print(f"✅ Accepted: {fname} (Malay: ✓, Educational: {'✓' if quality_check['educational_content'] else '✗'}, No watermark: ✓)")

    # Write enhanced metadata
    out_file = os.path.join(save_dir, 'ocr_metadata_malay.json')
    with open(out_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'processing_summary': {
                ...
                'malay_text_images': stats['malay_text'],
    ...
    print("\n🎯 ENHANCED MALAY OCR PROCESSING COMPLETE!")
    print("=" * 60)
    print(f"📊 Total images processed: {stats['total_processed']}")
    print(f"🔍 OCR successful: {stats['ocr_successful']}")
    print(f"🇲🇾 Malay text detected: {stats['malay_text']}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Filter and OCR Malay images using EasyOCR.'
    )
    parser.add_argument(
