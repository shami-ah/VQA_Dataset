...
def is_english_text(text: str, min_english_ratio: float = 0.7) -> bool:
    ...
    Args:
        ...
        min_english_ratio: Minimum ratio of English characters required
    ...
    english_chars = sum(1 for c in clean_text if c.isalpha() and ord(c) < 128)
...
def has_educational_keywords(text: str) -> bool:
    """
    Check if text contains educational keywords that indicate learning content
    ...
    educational_keywords = [
        'learn', 'study', 'education', 'school', 'lesson', 'teach', 'student',
        'math', 'science', 'physics', 'chemistry', 'biology', 'history',
        'geography', 'literature', 'grammar', 'vocabulary', 'equation',
        'formula', 'problem', 'solution', 'answer', 'question', 'exam',
        'test', 'quiz', 'homework', 'assignment', 'chapter', 'unit',
        'diagram', 'graph', 'chart', 'table', 'figure', 'example'
    ]
...
def is_high_quality_educational_image(image_path: str, ocr_text: str) -> Dict:
    """
    Comprehensive quality check for educational images
    ...
    # Check if predominantly English text
    assessment['english_text'] = is_english_text(ocr_text)
    if not assessment['english_text']:
        assessment['is_quality'] = False
        assessment['reasons'].append("Not predominantly English text")
...
def process_images(source_dir, save_dir):
    """
    Enhanced OCR-filter for English educational images with watermark detection.
    Only high-quality images with English educational text and no watermarks are saved.
    """
    ...
    print("🔍 Starting enhanced English OCR processing...")
    print("✅ Filtering for: English text + Educational content + No watermarks")
    ...
    for root, _, files in os.walk(source_dir):
        for fname in files:
            ...
            # Update statistics
            if quality_check['english_text']:
                stats['english_text'] += 1
            ...
            print(f"✅ Accepted: {fname} (English: ✓, Educational: {'✓' if quality_check['educational_content'] else '✗'}, No watermark: ✓)")
    ...
    with open(out_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'processing_summary': {
                ...
                'english_text_images': stats['english_text'],
    ...
    print("\n🎯 ENHANCED ENGLISH OCR PROCESSING COMPLETE!")
    print("=" * 60)
    print(f"📊 Total images processed: {stats['total_processed']}")
    print(f"🔍 OCR successful: {stats['ocr_successful']}")
    print(f"🇺🇸 English text detected: {stats['english_text']}")
