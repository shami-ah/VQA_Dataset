...
def is_bengali_text(text: str, min_bengali_ratio: float = 0.7) -> bool:
    """
    Check if text is predominantly Bengali
    
    Args:
        text: Text to analyze
        min_bengali_ratio: Minimum ratio of Bengali characters required
        
    Returns:
        True if text is predominantly Bengali
    """
    ...
    bengali_chars = sum(1 for c in clean_text if '\u0980' <= c <= '\u09FF')  # Bengali script
    ...
    return bengali_ratio >= min_bengali_ratio


def has_educational_keywords(text: str) -> bool:
    """
    Check if text contains Bengali educational keywords that indicate learning content
    ...
    educational_keywords = [
        'শেখা', 'পড়াশোনা', 'শিক্ষা', 'স্কুল', 'পাঠ', 'শেখানো', 'ছাত্র',
        'গণিত', 'বিজ্ঞান', 'পদার্থবিদ্যা', 'রসায়ন', 'জীববিদ্যা', 'ইতিহাস',
        'ভূগোল', 'সাহিত্য', 'ব্যাকরণ', 'শব্দভাণ্ডার', 'সমীকরণ',
        'সূত্র', 'সমস্যা', 'সমাধান', 'উত্তর', 'প্রশ্ন', 'পরীক্ষা',
        'পরীক্ষা', 'কুইজ', 'বাড়ির কাজ', 'অ্যাসাইনমেন্ট', 'অধ্যায়', 'একক',
        'ডায়াগ্রাম', 'গ্রাফ', 'চার্ট', 'টেবিল', 'চিত্র', 'উদাহরণ',
        'জ্ঞান', 'বিষয়', 'কোর্স', 'প্রফেসর', 'শিক্ষক', 'শিক্ষা'
    ]
...
def detect_watermarks(image_path: str, ocr_text: str) -> Tuple[bool, str]:
    ...
    getty_indicators = [
        'getty', 'gettyimages', 'getty images', 'ওয়াটারমার্ক'
    ]
    ...
    # Bengali watermark terms
    bengali_watermarks = [
        'ওয়াটারমার্ক', 'কপিরাইট', 'নমুনা', 'প্রিভিউ',
        'উদাহরণ', 'প্রদর্শনী', 'ডেমো'
    ]
    ...
    # Bengali watermark detection
    for watermark in bengali_watermarks:
        if watermark in ocr_text:  # Bengali text, case-sensitive
            return True, f"Bengali Watermark ({watermark})"
...
def is_high_quality_educational_image(image_path: str, ocr_text: str) -> Dict:
    """
    Comprehensive quality check for Bengali educational images
    ...
    # Check if predominantly Bengali text
    assessment['bengali_text'] = is_bengali_text(ocr_text)
    if not assessment['bengali_text']:
        assessment['is_quality'] = False
        assessment['reasons'].append("Not predominantly Bengali text")
...
def process_images(source_dir, save_dir):
    """
    Enhanced OCR-filter for Bengali educational images with watermark detection.
    Only high-quality images with Bengali educational text and no watermarks are saved.
    """
    os.makedirs(save_dir, exist_ok=True)

    # Initialize Bengali EasyOCR reader
    try:
        reader = easyocr.Reader(['bn'], gpu=False)  # Use CPU for better compatibility
    ...
    print("🔍 Starting enhanced Bengali OCR processing...")
    print("✅ Filtering for: Bengali text + Educational content + No watermarks")
    ...
    for root, _, files in os.walk(source_dir):
        for fname in files:
            ...
            # Update statistics
            if quality_check['bengali_text']:
                stats['bengali_text'] += 1
            ...
            print(f"✅ Accepted: {fname} (Bengali: ✓, Educational: {'✓' if quality_check['educational_content'] else '✗'}, No watermark: ✓)")

    # Write enhanced metadata
    out_file = os.path.join(save_dir, 'ocr_metadata_bengali.json')
    with open(out_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'processing_summary': {
                ...
                'bengali_text_images': stats['bengali_text'],
    ...
    print("\n🎯 ENHANCED BENGALI OCR PROCESSING COMPLETE!")
    print("=" * 60)
    print(f"📊 Total images processed: {stats['total_processed']}")
    print(f"🔍 OCR successful: {stats['ocr_successful']}")
    print(f"🇧🇩 Bengali text detected: {stats['bengali_text']}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Filter and OCR Bengali images using EasyOCR.'
    )
    parser.add_argument(
